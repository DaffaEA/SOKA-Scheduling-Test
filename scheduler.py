...
from dbo_algorithm import dbo_algorithm

# --- Konfigurasi Lingkungan ---
load_dotenv()

VM_SPECS = {
    'vm1': {'ip': os.getenv("VM1_IP"), 'cpu': 1, 'ram_gb': 1, 'mips': 1000},
    'vm2': {'ip': os.getenv("VM2_IP"), 'cpu': 2, 'ram_gb': 2, 'mips': 2000},
    'vm3': {'ip': os.getenv("VM3_IP"), 'cpu': 4, 'ram_gb': 4, 'mips': 4000},
    'vm4': {'ip': os.getenv("VM4_IP"), 'cpu': 8, 'ram_gb': 4, 'mips': 6000},
}
...
# DBO Algorithm Configuration
DBO_POPULATION = int(os.getenv("DBO_POPULATION", "30"))
DBO_ITERATIONS = int(os.getenv("DBO_ITERATIONS", "200"))
DBO_PROB_LOCAL = float(os.getenv("DBO_PROB_LOCAL", "0.7"))
...
def get_task_load(index: int) -> float:
    # Keep as float for fitness computations
...
async def main():
    ...
    for name, spec in VM_SPECS.items():
        ip = spec.get('ip')
        if not ip:
            print(f"Peringatan: IP untuk VM '{name}' tidak ditemukan di environment. Set ke 127.0.0.1 (uji lokal).", file=sys.stderr)
            ip = "127.0.0.1"
            missing_ip = True
        vms.append(VM(name, ip, spec['cpu'], spec['ram_gb'], spec['mips']))
    ...
    print(f"Menjalankan DBO Algorithm dengan {DBO_POPULATION} populasi, {DBO_ITERATIONS} iterasi...")
    # dbo_algorithm expects (tasks, vms, population, max_iter, prob_local)
    best_assignment = dbo_algorithm(tasks, vms, 
                                  population=DBO_POPULATION,
                                  max_iter=DBO_ITERATIONS,
                                  prob_local=DBO_PROB_LOCAL)
    
    if not isinstance(best_assignment, dict):
        print("Peringatan: dbo_algorithm tidak mengembalikan dictionary. Keluar.", file=sys.stderr)
        return

    # Print a sorted view of assignment
    print("\nPenugasan Tugas Terbaik Ditemukan (preview):")
    for task_id in sorted(best_assignment.keys())[:20]:
        vm_name = best_assignment[task_id]
        print(f"  - Tugas {task_id} -> {vm_name}")
    ...
    async with httpx.AsyncClient() as client:
        ...
        for task_id, vm_name in best_assignment.items():
            if task_id not in tasks_dict:
                print(f"Peringatan: task_id {task_id} ada di penugasan tetapi tidak ditemukan di tasks_dict. Mengabaikan.", file=sys.stderr)
                continue

            if vm_name not in vms_dict:
                print(f"Peringatan: vm '{vm_name}' yang ditetapkan ke task {task_id} tidak ada dalam konfigurasi VM. Mengabaikan tugas ini.", file=sys.stderr)
                continue

            task = tasks_dict[task_id]
            vm = vms_dict[vm_name]
            sem = vm_semaphores.get(vm_name, asyncio.Semaphore(1))
        ...
        # If there are no coroutines, skip
        if all_task_coroutines:
